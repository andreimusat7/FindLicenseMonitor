...
namespace FindLicenseStatus
{
    public partial class Monitor : Form
    {
        ...
        private void ReadFromResources()
        {
            ...
            label174.Text = Strings.StatusForm_PT_Total;
            label173.Text = Strings.StatusForm_PT;
            label175.Text = Strings.StatusForm_Nachshonim;
        ...
        private void CheckIfClosedMachine()
        {
            ...
            {
                if (row["Status"].ToString() == "VEP CLOSED" || row["Status"].ToString() == "OFF")
                {
                    try
                    {
                            ...
                            if (reply.Status.ToString() == "TimedOut")
                            {
                                ...
                                try
                                {
                                    ...
                                    using (SqlDataAdapter sqlAdap = new SqlDataAdapter(sqlCmd))
                                    {
                                        ...
                                        sqlCmd.CommandText = "UPDATE MachineStatus SET Status='ON' WHERE MachineName = '" + row["MachineName"].ToString() + "'";
                                        sqlCmd.Connection = sqlConn;
                                        sqlCmd.CommandType = CommandType.Text;
                                        sqlCmd.ExecuteNonQuery();
                                        sqlConn.Close();
                        ...
                        if (c.Text == item.MachineName) // Iterate through databse returned list
                        {
                            ...
                            foreach (Control z in c.Controls)
                            {
                                ...
                                if (z is Label)
                                {
                                    ...
                                    if (z.TabIndex == 2 && item.MachineStatus == "OFF")
                                        z.Text = "";
                                    if (z.TabIndex == 3)
